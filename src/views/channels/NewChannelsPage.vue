...
function channelBotId(channelKey: string): string {
  return readString(readChannelConfig(channelKey).botId)
}

function updateChannelBotId(channelKey: string, value: string): void {
  channelStore.setChannelField(channelKey, 'botId', shouldKeepStringValue(value))
}
...
async function installChannel(meta: ChannelCard): Promise<void> {
  ...
  try {
    ...
    // Set default values for Feishu channel
    if (meta.key === 'feishu') {
                ...
                <NCard v-if="card.configured" size="small" :title="t('pages.channels.basicConfigTitle')" embedded>
                  <NForm label-placement="left" label-width="140" class="channel-config-form">
                    ...
                    <NFormItem v-if="card.key === 'wecom'" :label="t('pages.channels.labels.botId')">
                      <NInput
                        :value="channelBotId(card.channelKey)"
                        :placeholder="t('pages.channels.placeholders.wecomBotId')"
                        @update:value="(value) => updateChannelBotId(card.channelKey, value)"
                      />
                    </NFormItem>
